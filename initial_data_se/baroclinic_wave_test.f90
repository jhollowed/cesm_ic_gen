...
!  Ullrich, Melvin, Jablonowski and Stabiforth (QJ, 2014) baroclinic instability.
!
...
!=======================================================================
  SUBROUTINE baroclinic_wave_test(deep,moist,pertt,X,lon,lat,p,z,zcoords,&
...
!-----------------------------------------------------------------------
    ...
    if (moist .eq. 1) then
      eta = p/p0 
      if (eta.gt.0.1_r8) then  ! intialize q if p > 100 hPa
        q = moistq0 * exp(- (lat/moistqlat)**4)          & 
                    * exp(- ((eta-1.d0)*p0/moistqp)**2)
      else
        q = 1.d-12              ! above 100 hPa set q to 1e-12 to avoid supersaturation
      endif
