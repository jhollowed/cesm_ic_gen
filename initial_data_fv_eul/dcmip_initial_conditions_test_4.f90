...
!=======================================================================
! use physical constants, already covered by module cam_physical_constants
!=======================================================================
...
CONTAINS
  ...
  SUBROUTINE test4_baroclinic_wave (lperturb,moist,X,hyam,hybm,lon,lat,p,z,zcoords,u,v,w,t,phis,ps,rho,q,q1,q2)
...
!-----------------------------------------------------------------------
    LOGICAL, INTENT(IN)  :: lperturb   ! if set to .TRUE. perturbation is added to trigger baroclinic wave 
    INTEGER, INTENT(IN)  :: moist      ! Moist (1) or dry (0) test case
    ...
    REAL(8), INTENT(IN)  :: &
                ...
                lat,        & ! Latitude (radians)
                z,          & ! Height (m)
                X             ! Scale factor for small Earth tests
    ...
    REAL(8) :: eta
    REAL(8) :: q_min_threshold = 1.d-7    ! minimum q in kg/kg for moist case
...
!-----------------------------------------------------------------------
    if (moist == 1) then
!       q = q0 * max(q_min_threshold,exp(-(lat/lat_hw)**4) * exp(-((eta-1.d0)*p0/p_hw)**2))  ! moist
       q = q0 * exp(-(lat/lat_hw)**4) * exp(-((eta-1.d0)*p0/p_hw)**2)  ! moist
       if (q .lt. q_min_threshold) q=q_min_threshold                   ! use a minimum threshold
...
!-----------------------------------------------------------------------
     q1 = theta(t,eta)
!    modify q1 if you want a different tracer distribution

!-----------------------------------------------------------------------
!     tracer q2, absolute value of EPV or constant
!-----------------------------------------------------------------------
!    modify q2 if you want a different tracer distribution
     if (abs(epv(lon,lat,eta)).lt.1.d-80) then
         q2 = 0.d0
      else 
...
!********************************************************************
! Temperature = mean + temperature deviation from mean
!********************************************************************
...
!********************************************************************  
  REAL(8) FUNCTION v_wind(lon,lat,eta,lperturb)
    ...
    REAL(8) :: phi_vertical, sin_tmp, cos_tmp, r, v_perturb
    REAL(8) :: perturb_lon, perturb_lat

    v_wind = 0.0d0

!
!   if you want perturb v, just remove the comments from the next block of codes
!   and comment out the wind perturnation for the u wind
!
!---------------
! if needed (lperturb == .TRUE.) add a Gaussian hill perturbation
!---------------
...
!
!       r = ACOS( sin_tmp + cos_tmp*COS(lon-perturb_lon) )         ! great circle distance without radius 'a'
!       v_perturb = perturbation_amplitude*EXP(- (r*radius)**2 )   !  perturbation_amplitude determines strength
!       v_wind    = v_perturb + v_wind                             ! perturbation + unperturbed u wind
!    ENDIF
...
!-----------------------------------------------------------------------
  REAL(8) FUNCTION epv(lon,lat,eta)
    ...
    if ( abs(1.d0 - K**2) .ge. 0.0000001) then
    ...
    zeta = -4.d0/a*cos_tmp*SIN(lat)*COS(lat)*(2.d0-5.d0*(SIN(lat))**2) &
       ...
       if (abs(lat) .lt. pi/2.0-0.00001 .and. lat .ne. -perturb_lat) then

       zeta = -4.d0/a*cos_tmp*SIN(lat)*COS(lat)*(2.d0-5.d0*(SIN(lat))**2) &
         + perturbation_amplitude/a* TAN(lat)
